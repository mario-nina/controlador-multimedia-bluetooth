...
/* Handles de los timers para cada LED */
...
/**
 * @brief Callback del timer para el LED azul.
 */
...
/**
 * @brief Callback del timer para el LED rojo.
 */
static void callback_timer_rojo(void *arg)
{
    estado_led_rojo = !estado_led_rojo;
    gpio_set_level(LED_ROJO, estado_led_rojo);
}

/**
 * @brief Detiene el timer activo de un LED y lo apaga.
 */
...
void control_leds_init(void)
{
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << LED_AZUL) | (1ULL << LED_ROJO),
...
void control_leds_set(led_id_t led, led_patron_t patron)
{
    ...
    switch (patron) {
        ...
        case LED_PARPADEO_LENTO: {
            uint64_t periodo_us = (patron == LED_PARPADEO_RAPIDO) ? LED_PERIODO_RAPIDO_US : LED_PERIODO_LENTO_US;
